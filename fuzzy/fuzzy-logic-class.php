...
class Fuzzify extends Member {
	...
	//AGREGADO POR GEYNEN
	public function getNameMember($idx,$id) {
		$i=0;
		foreach ($this->members[$idx] as $idx => $member){
			if($id==$i) $nombres=$member->FName;
			$i++;
		}
		return $nombres;
	}
	...
	//AGREGADO POR GEYNEN
	public function getNamesMember($idx) {
		foreach ($this->members[$idx] as $idx => $member){
			$nombres[]=$member->FName;
		}
		return $nombres;
	}		
...
/**
* agregado por: Geynen Montenegro Cochas
* Set Real Input Value @param2 for Input named @param1 
* example :
*    $fuzzy->getGradosPertenencia('input1',0.23);
* @param1   string $idx 
* @param2   float  $X
* return   none 
**/	

	public function getGradosPertenencia($idx,$X = 0.0) {
		$this->FRealInput[$idx]	=	$X;
		$this->FOutputs[$idx] = array();
		For ($i=0;$i<count($this->members[$idx]);$i++) {
			$this->FOutputs[$idx][]	=	$this->members[$idx][$i]->Fuzzification($this->FRealInput[$idx]); 
		}
		return $this->FOutputs[$idx];
	}
